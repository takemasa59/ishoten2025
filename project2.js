...
// ボタンの初期選択状態を設定
window.onload = () => {
    currentIndex = 0; // 初期状態は「商店街プロジェクト」
    updateImageAndDescription();

    // ボタンの初期選択状態を設定
    const buttons = document.querySelectorAll('.refbutton');
    buttons.forEach(button => button.classList.remove('selected')); // すべてのボタンから.selectedを削除
    document.getElementById('button1').classList.add('selected'); // 「商店街プロジェクト」のボタンを選択
